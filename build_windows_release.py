...
5. 首次运行会自动创建 `.env` 模板，请用记事本填写邮箱配置。
6. 在网页里点击“系统自检”，再点击“测试邮件”验证邮箱。

## 注意

- 不需要安装 Python。
- 不要打开 `.spec` 文件，它只是打包配置，不是启动程序。
- `products.xlsx` 用来给客户修改商品清单；如果误删，程序会尝试读取打包内置的初始商品清单。
- 不要把填写后的 `.env` 发给别人。
- 数据库、报告、备份和截图会保存在当前文件夹中。
- 如果网页提示“无法连接本地服务”，请重新双击 `护肤品价格助手.exe`，并确认地址栏是 `http://127.0.0.1:8765/`。
"""
